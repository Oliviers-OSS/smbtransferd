/* OC added memory error management */
/*-------------------------------------------------------------------------*/
/**
   @file	dictionary.c
   @author	N. Devillard
...
static void * mem_double(void * ptr, int size) {
    ...
    if (newptr) {
        ...
        errno = ENOMEM;
        ERROR_MSG("failed to allocated %d bytes of memory", (size * 2));
    }
...
dictionary * dictionary_new(int size) {
    ...
    if (!(d = (dictionary *) calloc(1, sizeof (dictionary)))) {
        errno = ENOMEM;
        ERROR_MSG("failed to allocate %d bytes of memory for the dictionary", sizeof (dictionary));
    } else {
        d->size = size;
        d->val = d->key = NULL;
        d->hash = NULL;
        ...
        if (d->val) {
            ...
            if (d->key) {
                ...
                if (d->hash) {
                    ...
                    errno = ENOMEM;
                    ERROR_MSG("failed to allocate %d bytes of memory for the hash", (size * sizeof (unsigned)));
                }
            } else {
                errno = ENOMEM;
                ERROR_MSG("failed to allocate %d bytes of memory for the key", (size * sizeof (char*)));
            }
        } else {
            errno = ENOMEM;
            ERROR_MSG("failed to allocate %d bytes of memory for the values", (size * sizeof (char*)));
        }

        if (errno != 0) {
            /* hash must be NULL (last allocation)*/
            if (d->key) {
                free(d->key);
                d->key = NULL;
            }
...
/*-------------------------------------------------------------------------*/
/**
  @brief	Enum keys in a given section
  @param	d		dictionary object to search.
  @param	section		Section to enum in the dictionary.
  @param        key             key found in this section
  @param        value           Value associated to the key in this section
  @param        position        Start position and next start position
  @return	status code, EXIT_SUCCESS if the section is found and a key in it
                , else ENOENT

  This function locates a key in a section of the dictionary and returns
  key, it's associated value and a position to allow iterations.
 */

/*--------------------------------------------------------------------------*/
int dictionary_enumkeys(dictionary * d, char *section,char **key, char **value,unsigned int *position) {
    unsigned int i;
    int error = ENOENT;
    char sec[20];
    const int n = sprintf(sec,"%s:",section);
    
    for (i = *position; i < d->n; i++) {
        if (d->key == NULL)
            continue;                

        /* Compare string beginning of the string */
        if (((strncasecmp(d->key[i],sec,n)) == 0) && (d->val[i] != NULL)){
            error = EXIT_SUCCESS;
            *key = d->key[i];
            *value = d->val[i];
            *position = i+1;
